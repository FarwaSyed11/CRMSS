                ...
                if (currentuser == "Sup") {
                    if (item.IsVerifiedBySup == 0) {
                        htm += `<span title='Add Comment'> <i class="fa fa-commenting-o fa-icon-hover icon-comment" aria-hidden="true" data-actid=` + item.Id + ` style="font-size:x-large; color:#d33a3a; cursor:pointer;"></i> </span>
                                <span title='Verify'>  <i class="fa fa-check-square-o fa-icon-hover icon-verify" aria-hidden="true" data-actid=` + item.Id + ` style="font-size:x-large; color:#d33a3a; cursor:pointer;margin-left:3px;"></i> </span>
`;
                    }
...
$('.tbody-employees').on('click', '.icon-verify', function () {

    selActId = $(this).data('actid');
    var selObjAct = listActivities.filter(s => s.Id == selActId)[0];
    ...
    if (rolesList.split(',').filter(s => s == 8104).length > 0) {
        ...
        var collObj = {};       
            collObj = {
                'Id': selObjAct.Id,
                "Remarks": selObjAct.SupervisorRemarks,
                'Role': currentuser,
                'AmntTobeCollected': selObjAct.ActualAmount == '' ? 0 : selObjAct.ActualAmount,
                'CollectedAmount': selObjAct.CollectedAmount == '' ? 0 : selObjAct.CollectedAmount,
                'EmpNo': EmpNo,
                'IsVerifiedBySup': 1,
                'IsVerifiedByMan': selObjAct.IsVerifiedByMan
            };
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    toastr.success('Action Verified Successfull', '');
                    objDatatable.destroy();
                    getAllActivity();
        ...
        var collObj = {};      
            collObj = {
                ...
                'CollectedAmount': selObjAct.CollectedAmount == '' ? 0 : selObjAct.CollectedAmount,
                'EmpNo': EmpNo,
                'IsVerifiedBySup': selObjAct.IsVerifiedBySup,
                'IsVerifiedByMan': 1
            };
...
$('.btnSaveComment').on('click', function () {
    ...
    $.ajax({
        url: "CalendarMaster.aspx/UpdateSupOrManagerRemarksFromReportSide",
        data: JSON.stringify({
            'Id': selActId,
