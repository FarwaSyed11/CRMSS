...
public partial class KamiraCosmetics_CalendarActivity : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Common.SaveAppUserActivityHistory(Session["ApplicationId"].ToString(), "/KamiraCosmetics/CalendarMaster.aspx", "Kamira Cosmetics Activity Area.", (Session["empno"] == null ? "null" : Session["empno"].ToString()), 0);
    }
        ...
        )
    {
        ...
        DBH.CreateDatasetForDBKamira(ds, "sp_CRMActivity", true, pa, pv);
    ...
    public static void InserActivitiesInBulk(List<Activity> data)
    {
        ...
        using (var table = new DataTable())
        {
            ...
            DBH.CreateDataset_ForBulkInsert(ds, "[sp_Bulk_AddCRMActivities]", "dbo.AddActivitiesList", table, "Kamira", true, pa, pv);
        }
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static List<DDL> GetMember(string UserID, string RolesList)
    {
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static List<DDL> LoadSector(string UserID)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            drpval.Add(new DDL()
            {
                Value = dt.Rows[i]["Sector"].ToString(),
                Name = dt.Rows[i]["Sector"].ToString()
            });
        }
        return drpval;
        //string a = userId;
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static List<DDL> LoadCountry()
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            drpval.Add(new DDL()
            {
                Value = dt.Rows[i]["Country"].ToString(),
...
//-------------------------------------------City
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static List<DDL> LoadCity(string Country)
    {
