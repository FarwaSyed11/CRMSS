...
var listDDLDepart = [], listDDLNation = [], listDDLLanguage = [], listRRF = [], listRRFReqAttach = [], listCandisForRFF = [], listCandiAttach = [];
...
var objDTRRF = [], objDTRRFReqAttach = [], objDTRRFReqAttachVO =[], objDTCandiRRF = [], objDTSrtlistedCandiRRF = [], objDTInterviewReqAttach = [], objDTCandiAttach = [], objDTInvCandiRRF = [], objDTSelectedCandiRRF = [], objDTHistoryRRF = [];
var sellanguages = '-1'; var isCordinator = 0;
...
function getUrlPara() {
    ...
    let rrfid = selRRFId;
    let selaction = 'Details RRF';


    let redUrl = window.location.origin + "/crmss/Recruitment/ApplicantTracking.aspx?rrfid=" + rrfid + "&selaction=" + selaction;
    window.open(redUrl);
}
$('#redirectToAppTrack').on('click', function () {
    getUrlPara();
})
...
$('.btnPrev,.btnNext').on('click', function () {
    ...
    if ($(this).text().trim() == 'Prev') {
        ...
        let stepNameByStepNo = stepNameArr.filter(s => s.StepNo == (stepIncorDec == 1 ? 1 : (stepIncorDec - 1)))[0].StepName;
...
function getAllRRF() {
    ...
    $.ajax({
        url: "Requisition.aspx/GetAllRRF",
        ...
        success: function (result) {
            ...
            $.each(listRRF, function (key, item) {
                ...
                if (isCordinator == 1) { htm += `<span style="margin-left: 4%;"><i class="bx bx-trash fa-icon-hover ibtn-rrf-delete" title="Delete RRF" data-rrfid="` + item.RRFId + `" style="color:#d33a3a; cursor:pointer;font-size: x-large;"></i> </span>` }
               // }
...
$('.tbody-rrf').on('click', '.ibtn-rrf-edit,.ibtn-rrf-details,.ibtn-rrf-delete', function () {
    ...
    if (selAction == "Details RRF") {
        ...
        //    loadSelectedCandiForRRF(); //load those candidates who are selected
            ...
            loadDocsHTMLForRRFReq();

            if (listApprover.filter(s => s.Status == 'APPROVED').length >= 4) { $('.apptrack-pagelink-div').removeClass('hidden') } else { $('.apptrack-pagelink-div').addClass('hidden') }
...
$('.btn-approve-rrfreq,.btn-reject-rrfreq').on('click', function () {
    bindEventAccordingly($(this)[0].title.trim()); //showing or hiding Approve and Reject Option
});
...
function checkForApproveButton() {
    ...
    let hodRoleCount = myroleList.split(',').filter(s => s == 13207).length;            // HOD
    ...
    else if (selRRFObj.ApproverOrderStatus == 2 && hrRoleCount > 0) {  //if recruiter
        $('.rrfreq-approve-div').removeClass('hidden');
    }
    else if (selRRFObj.ApproverOrderStatus == 3 && recruitRoleCount > 0) {  //if hr director
        $('.rrfreq-approve-div').removeClass('hidden');
    }
}

function bindEventAccordingly(paraSelAction) {
    var rrfForFilter = [];
    rrfForFilter.push(selRRFObj);

    $('.btnAppRejRRF').unbind('click');
    $('.btnAppRejRRF').html(paraSelAction);
    ...
    $('#appRejCommentModal').modal('show');
    $('#lblappRejCommentModal').html(paraSelAction == 'Approve' ? 'Approval Remarks' : 'Rejection Remarks');

    if (selRRFObj.ApproverOrderStatus == 0 && managerRoleCount > 0) {  // if manager
        $('.btnAppRejRRF').on('click', function () {
            let selAction = paraSelAction;
            selAction == 'Approve' ? SubmitOrApproveRRF(1, 'Request has been Approved', 'APPROVED', 13206) : SubmitOrApproveRRF(1, 'Request has been Rejected', 'REJECTED', 13206);
        });
    }
    else if (selRRFObj.ApproverOrderStatus == 1 && hodRoleCount > 0) {  // if hod
        $('.btnAppRejRRF').on('click', function () {
            let selAction = paraSelAction;
            selAction == 'Approve' ? SubmitOrApproveRRF(2, 'Request has been Approved', 'APPROVED', 13207) : SubmitOrApproveRRF(2, 'Request has been Rejected', 'REJECTED', 13207);
        });
    }
    else if (selRRFObj.ApproverOrderStatus == 2 && hrRoleCount > 0) {  //if recruiter
        $('.btnAppRejRRF').on('click', function () {
            let selAction = paraSelAction;
            selAction == 'Approve' ? SubmitOrApproveRRF(3, 'Request has been Approved', 'APPROVED', 13198) : SubmitOrApproveRRF(3, 'Request has been Rejected', 'REJECTED', 13198);
        });
    }
    else if (selRRFObj.ApproverOrderStatus == 3 && recruitRoleCount > 0) {  //if hr director
        $('.btnAppRejRRF').on('click', function () {
            let selAction = paraSelAction;
            selAction == 'Approve' ? SubmitOrApproveRRF(4, 'Request has been Approved', 'APPROVED', 13197) : SubmitOrApproveRRF(4, 'Request has been Rejected', 'REJECTED', 13197);
        });
    }
}
...
function SubmitOrApproveRRF(AppOrderStatus,msg,Status,roleid) {
    ...
    if ($('#taAppRejRemarks').val().trim() == '' && AppOrderStatus != 0) {
        toastr.error('Please Input the Remarks', '');
        $('#taAppRejRemarks').css('border', '2px solid #ff2c00');
    }
    else {
        $('#taAppRejRemarks').css('border', '');
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                'Status': Status,
                'RoleId': roleid,
                'Remarks': AppOrderStatus == 0 ? Status : $('#taAppRejRemarks').val()
            }),
            type: "POST",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            ...
            success: function (result) {
                ...
                getApproverList();
                getAllRRF();
                $('#appRejCommentModal').modal('hide'); $('#taAppRejRemarks').val('');
                $('.rrfreq-approve-div').addClass('hidden');
                $('.btnAppRejRRF').unbind('click');
                if (listApprover.filter(s => s.Status == 'APPROVED').length >= 4) { $('.apptrack-pagelink-div').removeClass('hidden') } else { $('.apptrack-pagelink-div').addClass('hidden') }
            },
...
function loadDocsForRRFReq(op) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('.tbody-rrfreq-attach td').length > 1 ? objDTRRFReqAttach.destroy() : '';
...
function loadDocsHTMLForRRFReq() {
    $.ajax({
        url: "Requisition.aspx/GetRRFReqAttachs",
        data: JSON.stringify({ 'RRFId': selRRFId }),
        type: "POST",
        ...
        success: function (result) {
            listRRFReqAttach = result.d;
            var htm = ''; var urlService = '';
            $('.tbody-rrfreq-attach-viewonly td').length > 0 ? objDTRRFReqAttachVO.destroy() : '';
            ...
            $('.tbody-rrfreq-attach-viewonly').html(htm);          
            initiateRRFReqAttacViewOnlyhDT();    
...
function initiateRRFReqAttacViewOnlyhDT() {
    objDTRRFReqAttachVO = [];
    objDTRRFReqAttachVO = $('.tbl-rrfreq-attach-viewonly').DataTable({
        dom: 'lBfrtip',
        "bStateSave": true,
        buttons: {
            buttons: [
...
$('.step-can-div-parent').on('click', '.step', function () {
   ...
   // else if (validateRRFFormControls(currActive.split('-')[1]) && validateRRFFormControls(gotoStepNo)) { //If go to directly any step so previous step must be completed of submitted
    else if (true) { //If go to directly any step so previous step must be completed of submitted
        $('.step-can-div-parent').find('.step-icon').removeClass('activestep');
        $(this).find('.step-icon').addClass('activestep');
...
$('.tbody-interview').on('click', '.ibtn-interviewtab-details,.ibtn-interviewtab-apprrej', function () {
    ...
    if (selAction == "Inv Evaluation") {
        ...
        $('#lblPosAppliedFor').html(selRRFObj.ReportTo)
        $('#txtInterviewer').html(selRRFObj.ReportTo)
        $('#lblInterviewDate').html(listInvCandisForRFF[0].FirstName)
...
$('.btnSubmitEvaluation').on('click', function () {
    ...
    if (validateEvaluationControls()) {
        ...
        $.each(dataList, function (key, item) {
            ...
            key == (dataList.length - 1) ? toastr.success('Evaluation Submitted Successfully', '') : '';
        })            
...
function updateCandiStatusAsSelected() {
    $.ajax({
        ...
        data: JSON.stringify({
            'CandiId': selCandiId,
            'RRFId': selRRFId,
            'CandiStatus': $('#rbCandiSelected:checked').val().trim(),
            'Salary': $('#txtCandiSalary').val(),
        ...
        success: function (result) {
            ...
            $.each(listHistoryRFF, function (key, item) {
                ...
                htm += `<tr> 
                    ...
                    <td> <span class="`+ getStatusColorForHistory(item.Remarks) + `">` + item.Remarks + `</span> </td>
                    </tr>`                    
...
function getStatusColorForHistory(param) {
    var colorClass = '';
    if (param.toUpperCase() == 'DRAFT') {
        colorClass = 'rrf-history-draft'
    }
    else if (param.toUpperCase() == 'SUBMITTED') {
        colorClass = 'rrf-history-draft'
    }
    else if (param.toUpperCase() == 'APPROVED') {
        colorClass = 'rrf-history-approved'
    }
    else if (param.toUpperCase() == 'REJECTED' || param.toUpperCase() == 'CANCELLED') {
...
function checkMyRole() {
    ...
    if (cordinatorRoleCount > 0) {
        $('#btn-add-rff-grid').removeClass('hidden');
        isCordinator = 1;
    }
    else {
        $('#btn-add-rff-grid').addClass('hidden');
        isCordinator = 0;
    }
    //else if (managerRoleCount > 0) {  // if manager
    //    $('#btn-add-rff-grid').addClass('hidden');
    //}
    //else if (hodRoleCount > 0) {  // if hod
    //    $('#btn-add-rff-grid').addClass('hidden');
    //}
    //else if (hrRoleCount > 0) {  //if recruiter
    //    $('#btn-add-rff-grid').addClass('hidden');
    //}
    //else if (recruitRoleCount > 0) {  //if hr director
    //    $('#btn-add-rff-grid').addClass('hidden');
    //}
