...
public partial class SalesManager_SMAccound : System.Web.UI.Page
{
	...
	protected void gdvSummaryDetails_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			...
			pv.Add(_Accountid);
				...
				DBH.CreateDatasetCRMEBSDATA(ds, "Sp_SMAccound", true, pa, pv);
				...
				if (ds.Tables[0].Rows.Count > 0)
				{
					...
					//Filteratration of Opportunity Gridview
        ...
        try
        {
            ...
            try
            {
                if (Convert.ToDouble(txtRelation.SelectedValue.Trim()) != 0 && txtRemarks.Text.Trim() != "" && ddlCustomerStatus.SelectedValue != "-1")
                {
                    ...
                    {
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "none",
                     " <script>  toastr.warning('If Relation Perc below to 70% CUstomer Status Should be Black List or To be Maintanined', 'Warning');</script>", false);
