...
public partial class KPI_KPIDashboard : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Common.SaveAppUserActivityHistory(Session["ApplicationId"].ToString(), "/KPI/KPIDashboard.aspx", "KPI Dashboard For Employee.", (Session["empno"] == null ? "null" : Session["empno"].ToString()), 0);
    }
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static ProfPic GetUserProfPic(string EmpNumber)
    {


        ProfPic empPic = new ProfPic();

        Ecl obj = new Ecl();

        DataTable dt = obj.BindUserDataForKPI(EmpNumber);

        if (dt.Rows.Count > 0)
        {
            empPic.PicUrl = dt.Rows[0]["UserImageLink"].ToString();
            empPic.EmpName = dt.Rows[0]["Full_name"].ToString();
        }
