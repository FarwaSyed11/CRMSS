...
$(document).ready(function () {
    ...
    //let emp = employeesList.filter(q => q.MyEmpNo == selEmpNo)[0];
    //profilpicurl = emp.EmpProPicUrl;
    //loggedInUserName = emp.MyName;

    //$('#dashboardProPic').attr("src", profilpicurl);
    //$('.loggedin-username').html(loggedInUserName);

    //loadingTeamsCardDiv(employeesList);


    var pageName = 'My Team Kpi Dashboard'
    activeCurrentPage(pageName, 0, 2);
});
...
function loadAllDashbaordValues() {
    ...
    selEmpNo = $('#ddlEmployee option:selected').val().trim();
    if (selEmpNo != "" && selEmpNo != "0") {
        selEmpDetails = employeesList.filter(q => q.MyEmpNo == selEmpNo);
        loadingTeamsCardDiv(selEmpDetails);
        //loadKpiPointsYearlyBasisFolAllUsers(selEmpNo);
    } else {
        loadingTeamsCardDiv(employeesList);
...
function loadYTDKpi() {
    ...
    $.ajax({
        url: "MyTeamKPIDashboard.aspx/GetYTDKpi",
...
function fillFiltersOfDashboard() {
    $.ajax({
        url: "MyTeamKPIDashboard.aspx/GetMyAndMyManagerDetails",
...
function loadEmployees() {
    ...
    $.ajax({
        url: "MyTeamKPIDashboard.aspx/GetEmployeesList",
        data: JSON.stringify({ EmpNumber: EmpNo }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadKPIAccToMonthYear() {
    $.ajax({
        url: "MyTeamKPIDashboard.aspx/GetKPIAccToMonthYear",
        data: JSON.stringify({ EmpNumber: selEmpNo, Month: selMonth, Year: selYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        async: false,
...
function loadKpiPointsYearlyBasis() {
    ...
    $.ajax({
        url: "MyTeamKPIDashboard.aspx/GetkpiPointsYearlyBasis",
...
function loadingTeamsCardDiv(empList) {


    var htm = '';
    $('.kpi-boxes-parent').html('');
    $.each(empList, function (key, item) {
        loadKpiPointsYearlyBasisFolAllUsers(item.MyEmpNo);
        ...
        htm = `<div class=" mb-2" style="width:16.6%;">
                                <div class="card">
                                    <div class="card-body row">
        ...
        htm += `</div>`;  // closing "card-title d-flex align-items-start justify-content-between col-md-10"
        //htm += `<div class="col-md-8"><label><b>YTD</b></label>  <label class="lblkpiYearToDate" style="color: ` + getDarkShade(YTDKPIListYearly.filter(s => s.KpiMasterId == item.kpiId)[0].YTD) + `">` + YTDKPIListYearly.filter(s => s.KpiMasterId == item.kpiId)[0].YTD + `% </label> </div>`;
        htm += `<div class="" style="width:54.3%;"> <label style="font-size:smaller;"><b>` + item.MyName + `</b></label>   </div>`;
