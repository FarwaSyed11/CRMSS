...
var objSurveyAdd = [], selQuesOptObj = [];
...
$(document).ready(function () {
    ...
    loadSurvey();
    $('.lbl-name-for-survey-para').html($('#lblFULLNAME').text().trim())     
    //$('#lblUser').text().trim() =='NA2570'? $('#askingForSurveyModal').modal('show'):'';
    ...
    $('.btn-survey-yes').on('click', function () {
        ...
        $('#confirmNA').modal('hide');
         ...
         // changing Survey name and Category based on question
    ...
    $('.btnSaveSurvey').on('click', function () {
        let isValid =true; $('.fuload-file-label').html('');

        $('.tbody-linemanager-list > tr').each(function() {        
            
            let starRate = $(this).children().eq(2).children().children().eq(0).text().trim();
            let Na = $(this).children().eq(3).children().children().is(':checked');

            if(starRate == 0 && Na == false) {toastr.error('Please Rate Line Managers before submission.', ''); isValid=false; return false;}
        })
    ...
    $('.tbody-linemanager-list').on('change', 'input[type=checkbox]', function () {    
        ...
        if ($(this).is(':checked')) {
            ...
            $(this).parent().parent().parent().children().eq(2).children().addClass('hidden');
        } else {
            ...
            $(this).parent().parent().parent().children().eq(2).children().removeClass('hidden')
        }
        savingRatingForManager();
    });


    $('#fu-task-attach').change(function () {
        $('.fuload-file-label').html($('#fu-task-attach').get(0).files[0].name);
    });
...
function gotoNext() {
    ...
    if(quesStep == ($('.q-content-div-parent .q-content-div').length + 1))
    {
        ...
        $('#finalStepDetailsModal').modal('show');
    ...
    //quesStep == listSurveyQuestions.length+1 ? $('.btn-surv-next').addClass('hidden')
}
...
function gotoPrev() {
    ...
    if(quesStep <= listSurveyQuestions.length) { $('.btn-surv-next').removeClass('hidden'); $('.btnSaveSurvey').addClass('hidden')}
}
...
function savingRatingForManager() {
    ...
    $.ajax({
        url: "Econnect.aspx/SaveRatingForManager",
        data: JSON.stringify({
            "EmpNo": $('#lblUser').text().trim(),
...
function getLineManagersForSurvey() {
    $.ajax({
        url: "Econnect.aspx/GetLineManagersForSurvey",
        data: JSON.stringify({ "SurveyId": objMySurvey.length == 0 ? listSurveyQuestions[0].SurveyId : objMySurvey[0].SurveyId }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (result) {
...
function loadSurvey() {
    $.ajax({
        url: "Econnect.aspx/GetSurvey",
        data: JSON.stringify({ "EmpNo": $('#lblUser').text().trim() }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (result) {
            ...
            buildSurveyForm();
            //competency = listSurveyQuestions[0].Category;
            listSurveyQuestions.length > 0 ? getLineManagersForSurvey() : '';
        },
        error: function (errormessage) {
            alert(errormessage.responseText);
        }
...
function addUpdateSurvey() {
    ...
    $.ajax({
        url: "Econnect.aspx/AddUpdateSurvey",
...
function GetDetailsForSurveyAvailability() {
    $.ajax({
        url: "Econnect.aspx/GetDetailsForSurveyAvailability",
...
function saveSurveyStatus(st) {
    ...
    $.ajax({
        url: "Econnect.aspx/SaveSurveyStatus",
        data: JSON.stringify({
            "EmpNo": $('#lblUser').text().trim(),
            "SurveyId": objMySurvey[0].SurveyId,
            "ActionTakenByEmp": st,
...
function uploadSurveyAttach() {
    ...
    $('.fuload-file-label').html(files[0].name)
    var qrystringLocal = 'https://crmss.naffco.com/CRMSS/Services/FileUploadHandler.ashx?fufor=survey&surveyid=' + objMySurvey[0].SurveyId + '&crdate=' + FormateDateWithTime(new Date()) + '&comment=' + $('#taGeneralFeedback').val().trim() + '&empno=' + $('#lblUser').text().trim();    //for Live
