<%@ WebHandler Language="C#" Class="AssetAccessfileUpload" %>

using System;
using System.Collections;
using System.Collections.Generic;
using System.Data;
using System.IO;
using System.Linq;
using System.Web;
using System.Configuration;
using System.Data.SqlClient;

public class AssetAccessfileUpload : IHttpHandler {

 string AssetId = HttpContext.Current.Request.QueryString["ReqID"];
    
    public void ProcessRequest(HttpContext context)
{
    context.Response.ContentType = "text/plain";
    //context.Response.Write("Hello World");

    try
    {
            string pathAndFile = "";


            //commented this becasu for now due to security it will not allowing us to create any directory into crmss folder
            //string dirFullPath = HttpContext.Current.Server.MapPath("~/Images/Collection");
            //string[] files;
            //int numFiles;
            //files = System.IO.Directory.GetFiles(dirFullPath);  //for reading all files from server path
            //numFiles = files.Length;
            //numFiles = numFiles + 1;
            string str_image = "";

            foreach (string s in context.Request.Files)
            {
                HttpPostedFile file = context.Request.Files[s];
                string fileName = AssetId + " - " + file.FileName;
                string fileExtension = file.ContentType;

                if (!string.IsNullOrEmpty(fileName))
                {
                    //fileExtension = Path.GetExtension(fileName);
                    //file.SaveAs(System.IO.Path.Combine(HttpContext.Current.Server.MapPath("~/Images/KPI/KpiResponse"), fileName));

                    pathAndFile = "\\\\zylab\\Econnect\\SSHR\\AssetAccess";
                    pathAndFile += AssetId + " - " + file.FileName;
                    file.SaveAs(pathAndFile);
                }
            }


                DBHandler DBH = new DBHandler();
                DataSet ds = new DataSet();
                DataTable dt = new DataTable();
                ArrayList pa = new ArrayList();
                ArrayList pv = new ArrayList();

                pa.Add("@oper");
                pv.Add(3);

                pa.Add("@ReqID");
                pv.Add(AssetId);

                pa.Add("@FileName");
                pv.Add(pathAndFile);


                DBH.CreateDataset_SSHR(ds, "sp_AssetnAccess", true, pa, pv);
}
     }       

 
    public bool IsReusable {
        get {
            return false;
        }
    }

}