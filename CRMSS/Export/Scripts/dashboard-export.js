...
$(document).ready(function () {
    ...
    //var countformarkRole = roleListData.filter(s => s.Roles == '2088').length;
    //if (countformarkRole > 0) {
    //    window.location.pathname = '/CRMSS/Marketing/MarketingDashboard.aspx';
    //    return false;
    //}
    ...
    //var countforGM = roleListData.filter(s => s.Roles == '4094').length;
    //var countforsaleManagerRole = roleListData.filter(s => s.Roles == '2090').length;

    //if (countforbothRole == 2) {
    //    //var url = 'CRMSS/Dashboard/SaleDashboard.aspx';
    //    //window.location.href.split('CRMSS')[0] + url;
    ...
    $('.map-class').on('click', function () {
        //map.off();
        //map.remove();
        //var homeTab = document.getElementById('navs-t50-tender');
...
function getAllDashboardDataValues() {
    ...
    //let selectedYear = new Date().getMonth() == 0 ? (parseInt(ddlYearId.val()) - 1) : ddlYearId.val();
    let currLoginUser = currUserId.val().trim();
    let userStatus = $('#ddlActiveOrDeactiveUser option:selected').val().trim();

    loadForCountsAndValue(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    //for cards
    ...
    //loadNumbersOfJOH(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);   //comment now
   // loadNumbersOfFinalized(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);  //comment now
    //loadNumberOfSalesForecast(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);   //comment now
    //loadPipline(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus); // comment now
    //loadFinalPrice(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    //getCurrentForcastedOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);  //comment now
    //getFutureForcastedOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);   //comment now
    //getPendingForcastedOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);  //comment now

    //loadBChartValForFinalCard(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);   //comment now
    //loadBChartValForPipelineCard(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);//comment now

    //load for chart value
   // loadWinLostData(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus, "0");  // comment now
    ...
    //loadBarRevenueChartTheme();  //comment now
   /// loadGrowthChartTheme();
    //loadFinalizationbarChart();   // comment now
    //loadPipelinebarChart();    // comment now
    ///loadForecastOppChart();
    /// loadFutureForecastOppChart();
    //loadGrowthChartCRMSOverAll();  //comment now
    ...
    //loadOpenOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus); // comment now
    ////loadTop50Tender(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    //loadOverview(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);  //comment now

    //chart calling
    //loadOverViewRoundChart();   //comment now
...
function loadDDLCompany() {
    ...
    $.ajax({
        url: "DashboardExport.aspx/LoadCompanyDropdown",
        type: "POST",
...
function loadDDLManager(compId, uId) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/LoadManagerDropdown",
        type: "POST",
...
function loadDDLSal(uId, compId, manId) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/LoadSalemanDropdownExport",
        type: "POST",
...
function loadWinLostData(compId, managerId, salesmanId, uId, year, uStatus, oper) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetWonLostgraph",
...
function loadWonPerDataByYear(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetWonPerc",
...
function loadForCountsAndValue(compId, managerId, salesmanId, uId, year, uStatus) {

    $.ajax({
        url: "DashboardExport.aspx/GetCountnvalForGCs",
        ...
        async: true,
        success: function (result) {
            dashboardObjectList = result.d;

            $.each(result.d.listFirstRowRoundChrt, function (key, item) {

                if (item.For == "YTDSo") {
                    arrAchiechedPercValueForGC1 = [];
                    arrAchiechedPercValueForGC1.push(parseInt(item.Perc == "" ? 0 : item.Perc));
                    arrForGCSaleEmp1.push(parseInt(item.Value == 0 ? 0 : item.Value));
                    $('#g-chart-crms1').html(nFormatter(parseInt(item.Target == "" ? 0 : item.Target)));
                    overAllPerfo = overAllPerfo + parseFloat(item.Overall == "" ? 0 : item.Overall);
                }
                else if (item.For == "YTDInv") {
                    arrAchiechedPercValueForGC2 = [];
                    arrAchiechedPercValueForGC2.push(parseInt(item.Perc == "" ? 0 : item.Perc));
                    arrForGCSaleEmp2.push(parseInt(item.Value == 0 ? 0 : item.Value));
                    $('#g-chart-crms2').html(nFormatter(parseInt(item.Target == "" ? 0 : item.Target)));
                    overAllPerfo += overAllPerfo + parseFloat(item.Overall == "" ? 0 : item.Overall);
                }
                else if (item.For == "YTDColl") {
                    arrAchiechedPercValueForGC3 = [];
                    arrAchiechedPercValueForGC3.push(parseInt(item.Perc == "" ? 0 : item.Perc));
                    arrForGCSaleEmp3.push(parseInt(item.Value == 0 ? 0 : item.Value));
                    $('#g-chart-crms3').html(nFormatter(parseInt(item.Target == "" ? 0 : item.Target)));
                    overAllPerfo += overAllPerfo + parseFloat(item.Overall == "" ? 0 : item.Overall);
                }
                else if (item.For == "YTDUpdate") {
                    arrAchiechedPercValueForGC6 = [];
                    arrAchiechedPercValueForGC6.push(parseInt(item.Perc));
                    arrForGCSaleEmp6.push(nFormatter(parseInt(item.Value)));
                    $('#g-chart-crms6').html(parseInt(item.Target));
                    overAllPerfo += overAllPerfo + parseFloat(item.Overall);
                }
            ...
            // ----------- For Win and Loss Graph Year basis --------------------- //
            profitArray = [];
            lossArray = [];
            MonthArrayGraph = [];
            let sumOfWon = 0, sumOfLost = 0;
            yearPercValueForGrowthGraph = 0;
            let oper = 0;
            $.each(result.d.listWonAndLostGraph, function (key, item) {
            ...
            loadBarRevenueChartTheme();
            loadGrowthChartTheme();
            // ----------- For Win and Loss Graph Year basis Product wise--------------------- //
            // will have to do
            //end


            //------------- Forcecast Box value count ----------------------//
            $('.sales-forecast-value-card').html(result.d.listForecastBox[0].SalesForecastValue == "" ? 0 : nFormatter(result.d.listForecastBox[0].SalesForecastValue));
            $('.sales-forecast-value-card-count').html(result.d.listForecastBox[0].SalesForecastedCount == "" ? 0 : parseInt(result.d.listForecastBox[0].SalesForecastedCount).toLocaleString("en-US"));


            //------------- Number of Open JOH value count ----------------------//
            $('.no-joh-count').html(parseInt(result.d.listOpenJOHBox[0].OTPCount).toLocaleString("en-US"));
            $('.no-joh-value').html(nFormatter(result.d.listOpenJOHBox[0].OTPValue));


            //------------- Country APEX Graph ----------------------//
            arrayForFinalizedBarChartX_Axis = []; arrayForFinalizedBarChartY_Axis = [];
            var totalCountryVal = 0;
            $.each(result.d.listCountryBoxGraph, function (key, item) {
                ...
                totalCountryVal += parseInt(item.FinalizedValue);
            });
            loadFinalizationbarChart();
           // $('.no-of-finalized-count').html(nFormatter(totalCountryVal))

           //------------- Pipline APEX Graph ----------------------//
            arrayForPipelineBarChartX_Axis = []; arrayForPipelineBarChartY_Axis = [];
            var totalPipeVal = 0;
            $.each(result.d.listPipelineBoxGraph, function (key, item) {
                ...
                totalPipeVal += parseInt(item.PipelineValue);
            });
            loadPipelinebarChart();
            //$('.pipeline-value-card').html(nFormatter(totalPipeVal));
            //----------- Overview ---------------------------//
            arrOverViewChart = [];

            var item = result.d.listOverviewLastRowBoxes[0];
            ...
            arrOverViewChart.push(parseInt(item.SecuredValue));
            arrOverViewChart.push(parseInt(item.UnderRiskValue));
            arrOverViewChart.push(parseInt(item.EarlyToJudgeValue));
            arrOverViewChart.push(parseInt(item.LostValue));
            loadOverViewRoundChart();


            //------------------- Previous Forecast -------------------------------//
            let preFcastHtml = '';
            if (result.d.listPrevForecast.length == 0) { preFcastHtml = '<div> <h4 style="text-align:center;"> No Forecast Found <i class="fa fa-exclamation" aria-hidden="true"></i> </h4> </div>';  } 
            $.each(result.d.listPrevForecast, function (key, item) {

                preFcastHtml += `<li class="d-flex mb-4 pb-1">
            ...
            $('.forcasted-pending-opportunity-ul').html(preFcastHtml);


            //------------------- Current Forecast -------------------------------//
            let CurrFcastHtml = '';
            if (result.d.listCurrentForecast.length == 0) { CurrFcastHtml = '<div> <h4 style="text-align:center;"> No Forecast Found <i class="fa fa-exclamation" aria-hidden="true"></i> </h4> </div>'; /*$('.curr-fcast-total-heading').html('')*/ } 
            $.each(result.d.listCurrentForecast, function (key, item) {

                CurrFcastHtml += `<li class="d-flex mb-4 pb-1">
                    <div class="avatar flex-shrink-0 me-3">
            ...
            $('.forcasted-opportunity-ul').html(CurrFcastHtml);

            //------------------- Future Forecast -------------------------------//
            let futurFcastHtml = '';
            if (result.d.listFuturForecast.length == 0) { futurFcastHtml = '<div> <h4 style="text-align:center;"> No Forecast Found <i class="fa fa-exclamation" aria-hidden="true"></i> </h4> </div>'; /*$('.futur-fcast-total-heading').html('')*/ } /*else $('.futur-fcast-total-heading').html('Total Value');*/
            $.each(result.d.listFuturForecast, function (key, item) {

                futurFcastHtml += `<li class="d-flex mb-4 pb-1">
                    <div class="avatar flex-shrink-0 me-3">
            ...
            $('.forcasted-futur-opportunity-ul').html(futurFcastHtml);



            //-------------------- Top 50 Open JOH ------------------------------//            
            var topOpp = '';
            $.each(result.d.listTop50Opp, function (key, item) {
...
function loadSalesCalForGC1(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetSalesCalForGC1",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC2(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetSalesCalForGC2",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC3(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetSalesCalForGC3",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC4(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetSalesCalForGC4",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC5(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetSalesCalForGC5",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC6(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetSalesCalForGC6",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadUserDetails(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetUserImage",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadNumberOfSalesForecast(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        ...
        url: "DashboardExport.aspx/GetNumbersOfSalesForecast",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadFinalPrice(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetFinalPrice",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadNumbersOfJOH(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetNumbersOfJOH",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadNumbersOfFinalized(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetNumbersOfFinalized",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadBChartValForFinalCard(compId, managerId, salesmanId, uId, year, uStatus) {
    arrayForFinalizedBarChartX_Axis = []; arrayForFinalizedBarChartY_Axis = [];
    $.ajax({
        url: "DashboardExport.aspx/GetBarChartValueForFinalizedCard",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadPipline(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetPiplineValue",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadBChartValForPipelineCard(compId, managerId, salesmanId, uId, year, uStatus) {
    arrayForPipelineBarChartX_Axis = []; arrayForPipelineBarChartY_Axis = []
    $.ajax({
        url: "DashboardExport.aspx/GetBarChartValueForPipelineCard",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadOverview(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetOverview",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getValueForProductOrValueBar(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetValForLostByCompitOrValue",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadOpenOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetTopOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadTop50Tender(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "DashboardExport.aspx/GetTopOpportunitiesTender",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getCurrentForcastedOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    arrForecastOppChart = [];
    $.ajax({
        url: "DashboardExport.aspx/GetCurrentForcastedOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getPendingForcastedOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    //arrForecastOppChart = [];
    $.ajax({
        url: "DashboardExport.aspx/GetPendingForcastedOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getFutureForcastedOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    arrFutureForecastOppChart = [];
    $.ajax({
        url: "DashboardExport.aspx/GetFuturForcastedOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function drawMarkOnMap() {
    ...
    $.each(dashboardObjectList.listCountryForMap, function (key, item) {

        var htm = `<div style="text-align:center;"> <b style="color:#cd5050">` + item.CountryName + ` </b></div>`;
        //<div><span>Count: </span> <span style="color: #e36535;">`+ item.Value + `</span></div>
        htm += `<div> <span>Value: </span> <span style="color: #e36535;">` + nFormatter(item.Value) + ` </span><div>`;
        if (item.Name != '') {
            ...
            var mm = L.marker([item.Lat, item.Long]).addTo(map);
