...
function LoadCompanyDDL() {
    debugger;
    $.ajax({
        url: "AdminActionTender.aspx/GetCompanyDDL",
...
function LoadSalesmanager() {
    debugger;
    $.ajax({
        url: "AdminActionTender.aspx/GetSalesmanager",
...
function LoadSalesman() {
    debugger;
    $.ajax({
        url: "AdminActionTender.aspx/GetSalesman",
...
function LoadAdminAction(loadername) {
    ...
    //$('.ajax-loader').show();
    $.ajax({
        url: "AdminActionTender.aspx/GettableData",
 ...
 <td style="text-align:center"> <span style="word-wrap: break-word;max-width: 180px;display: inline-block;">`+ item.PlotNo + `</span></td>      
...
function othersUpdates(prjNumber) {
    ...
    $.ajax({
        url: "AdminActionTender.aspx/GetOthersUpdates",
        ...
        async: false,
        success: function (result) {
            $('.tbody-OPTList td').length > 0 ? objDatatableOppDet.destroy() : '';
            var htm = '';
            var htmsm = '';
            var htmm = '';
            var htmrev = '';
...
function initiateDataTablOppDetails() {
    objDatatableOppDet = [];
    objDatatableOppDet = $('.Opp-List').DataTable({
        dom: 'lBfrtip',
        buttons: {
            buttons: []
        },
...
function UpdateAction(AdminAction, AdminRemarks, selRefNumber) {
    ...
    $.ajax({
        url: "AdminActionTender.aspx/UpdateAction",
...
$('#btnExport').on('click', function () {
    window.open('../Services/AdminActionExport.aspx?Company=' + company + '&Manager=' + manager + '&Salesman=' + salesman + '&oper=' + 2 + '&OverView=' + $('#ddlOverview').val() + '&Action=' + Action)
});