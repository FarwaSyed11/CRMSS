        ...
        htm += `<div class="accordion-item">
                            <div class="card">
                                <h2 class="card-header" id="heading`+ item.prjNumber + `-` + item.StructureID + `">
                                    <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse` + item.StructureID + `" aria-expanded="false" aria-controls="collapse` + item.StructureID + `" style="color: #000000;text-transform: capitalize;">` + (key + 1) + `.  ` + item.StructureName + `<span class="position-absolute" style="right: 80px;"> Total Floors:  ` + item.TotalNumberOfFloor + `</span> <span class="position-absolute" style="right: 214px;"> <i class="bx bx-edit-alt struct-edit" style="color:#1eb0d0;font-size: 1.9rem;" onclick="openModalEditStruct(` + item.StructureID +`)"></i></span></button>
                                 </h2>
