        ...
        success: function (result) {
            ...
            $.each(listSystems, function (key, item) {
                if (key == 0) { // for first record only
                    ...
                    htm += `<div style="background: #fcfafa; text-align: center; border: 1px solid #b70000; color: black; border-radius: 20px 20px 3px 3px; font-weight: 600; padding: 15px 0px 15px 0px;">` + item.SysName + ` <span style="float: right;margin-right: 35px;cursor:pointer;"><i class='bx bxs-dollar-circle' onclick='openEngrNTestCommisionModal("` + item.SysName +`")'></i> </span></div>`;
                ...
                for (var i = 0; i < AllCategoryForSys.length; i++) {
                    ...
                    $.each(res, function (k,CatItem) {
                        ...
                        if (res[0].Category == "PIPES") {
                            htm += `<td> ` + CatItem.PipeUnitPrice + `</td>
                                    <td> `+ CatItem.FittingsPerc +`</td > 
                                    <td> `+ CatItem.InstallUnitPrice + `</td>`                                    
                        } 

                        htm +=    `<td> `+ CatItem.SpareQuantity +`</td>
                                   <td> <a class="ibtn-addfloor-into-items" title="Add Floors" data-estiid="`+ CatItem.EstiLineId + `" data-itemcode="` + CatItem.ItemCode + `"> <svg xmlns="http://www.w3.org/2000/svg" width="2rem" height="2rem" viewBox="0 0 24 24" onclick="openAddFloorModal(` + CatItem.EstiLineId + `,` + CatItem.ItemCode + `,'` + CatItem.Category +`')">
