...
$(document).ready(function () {
    ...
    //    monthNames.push(i);
    //}
    $('#DDLEmployee').val(new Date().getMonth() + 1);
    getDaysInMonth();
...
function getAllFuncOfDashboard() {
    ...
    $('.lblCurrentDate').html($('#DDLEmployee option:selected').text().toUpperCase());

    loadEfficencyGraph(currUserId.val(), cMonth, cYear);
    loadUserDetails(currUserId.val());
    getAllCount(currUserId.val(), cMonth, cYear);
    getCurrentmonthTaskList(currUserId.val(), cMonth, cYear);
    getCurrentmonthTaskListForOverview(currUserId.val(), cMonth, cYear);
    getPreviousPendingTaskList(currUserId.val(), cMonth, cYear);
    getFutureTaskList(currUserId.val(), cMonth, cYear);
   ...
   // getCurrentDate();
    ...
    //EfficencyCalendar();
    loadOverAllEfficency(currUserId.val());
    growthchart();
}
...
function getDaysInMonth() {
    monthNames = [];
    cMonth = new Date($('#DDLEmployee option:selected').val()).getMonth() + 1;
...
function getAllCount(uId, cMonth, cYear) {
    ...
    $.ajax({
        url: "DAshboard.aspx/CurrentmonthCompletedTask",
...
function getCurrentmonthTaskListForOverview(uID) {
    ...
    $.ajax({
        url: "DAshboard.aspx/CurrentmonthTaskListForOverview",
...
function getCurrentmonthTaskList(uID) {
    ...
    $.ajax({
        url: "DAshboard.aspx/CurrentmonthTaskList",
        data: JSON.stringify({ "EmpNum": EmpNo, "currentMonth": cMonth, "currentYear": cYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
{
    $.ajax({
        url: "DAshboard.aspx/PreviousPendingTaskList",
        data: JSON.stringify({ "EmpNum": EmpNo, "currentMonth": cMonth, "currentYear": cYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
{
    $.ajax({
        url: "DAshboard.aspx/FutureTasksList",
        data: JSON.stringify({ "EmpNum": EmpNo, "currentMonth": cMonth, "currentYear": cYear  }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadEfficencyGraph(uID) {
    ...
    $.ajax({
        url: "Dashboard.aspx/MonthlyEfficencyGraph",
        ...
        success: function (result) {
            ...
            for (var i = 1; i <= daysInCurrentMonth; i++) {
                ...
                if (result.d.filter(t => t.Day == i).length > 0) {
                    ...
                    title = 'PENDING';
                    mDate = ($('#DDLYear option:selected').val()) + '-' + ($('#DDLEmployee option:selected').val() < 10 ? '0' + $('#DDLEmployee option:selected').val() : $('#DDLEmployee option:selected').val()) + '-' + (i < 10 ? '0' + i : i);
                    eColor = '#e35d5d'
                }
...
function EfficencyCalendar() {
    ...
    function loadCalendar() {
        ...
        for (let i = 1; i <= countDays; i++) {
            const day01 = {
                day: i - paddingDays,
                month: $('#DDLEmployee option:selected').val(), //month + 1,
                year: year,
        ...
        var htm = '[';
        let currMonth = $('#DDLEmployee option:selected').val();
...
function loadOverAllEfficency(uID) {
    ...
    $.ajax({
        url: "DAshboard.aspx/GetOverallEfficiency",
        data: JSON.stringify({ "userId": uID, "currentMonth": cMonth, "currentYear": cYear  }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function initializedFullCal() {
   ...
   // document.addEventListener('DOMContentLoaded', function () {
    ...
    oFullCalendar = new FullCalendar.Calendar(calendarEl, {
        ...
        initialDate: ($('#DDLYear option:selected').val()) + '-' + ($('#DDLEmployee option:selected').val() < 10 ? '0' + $('#DDLEmployee option:selected').val() : $('#DDLEmployee option:selected').val()),
