...
var employeesList = [], lManagerList = [], hodList=[];
var selUserId = '';
...
function getAllFuncOfDashboard() {
    ...
    if ($('#DDLLineMan option:selected').val() == 'NO MANAGER AVAILABLE' || $('#DDLLineMan option:selected').val() == 'FILTER NOT INCLUDE') {
        $('#DDLEmployees').val('');
        EmpNo = $('#DDLHod option:selected').val();
        selUserId = hodList.filter(s => s.EmpNo == EmpNo)[0].UserId;
    }
    else if ($('#DDLEmployees option:selected').val() == 'NO EMPLOYEE AVAILABLE' || $('#DDLEmployees option:selected').val() == 'FILTER NOT INCLUDE') {
...
function getAllCount(uId, cMonth, cYear) {
    ...
    $.ajax({
        url: "DashboardAdmin.aspx/CurrentmonthCompletedTask",
...
function getCurrentmonthTaskListForOverview(uID) {
    ...
    $.ajax({
        url: "DashboardAdmin.aspx/CurrentmonthTaskListForOverview",
...
function getCurrentmonthTaskList(uID) {
    ...
    $.ajax({
        url: "DashboardAdmin.aspx/CurrentmonthTaskList",
        data: JSON.stringify({ "EmpNum": EmpNo, "currentMonth": cMonth, "currentYear": cYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getPreviousPendingTaskList(uID) {
    $.ajax({
        url: "DashboardAdmin.aspx/PreviousPendingTaskList",
        data: JSON.stringify({ "EmpNum": EmpNo, "currentMonth": cMonth, "currentYear": cYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getFutureTaskList(uID) {
    $.ajax({
        url: "DashboardAdmin.aspx/FutureTasksList",
        data: JSON.stringify({ "EmpNum": EmpNo, "currentMonth": cMonth, "currentYear": cYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadEfficencyGraph(uID) {
    ...
    $.ajax({
        url: "DashboardAdmin.aspx/MonthlyEfficencyGraph",
...
function loadUserDetails(uID) {
    ...
    $.ajax({
        url: "DashboardAdmin.aspx/GetUserImage",
...
function loadOverAllEfficency(uID) {
    ...
    $.ajax({
        url: "DashboardAdmin.aspx/GetOverallEfficiency",
        data: JSON.stringify({ "userId": uID, "currentMonth": cMonth, "currentYear": cYear }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function fillOrgDDL() {
    $.ajax({
        url: "DashboardAdmin.aspx/GetOrganizations",
...
$('#DDLDept').on('change', function () {
    selDep = $('#DDLDept option:selected').val().trim();
    fillHOD_DDL();
});
...
$('#DDLLineMan').on('change', function () {
    selLineMan = $('#DDLLineMan option:selected').val().trim();
    fillEmployee_DDL();
});
$('#DDLEmployees').on('change', function () {
    selEmpNo = $('#DDLEmployees option:selected').val().trim();
    // fillLineManDDL();
});

function fillDepartDDL() {
    $.ajax({
        url: "DashboardAdmin.aspx/GetDapartments",
        ...
        success: function (result) {
            ...
            res.sort(function (a, b) {
                return (a).DeptName.toUpperCase().localeCompare((b).DeptName.toUpperCase());
            })
            $.each(res, function (idx, itm) { sortDeptName.push(itm) });


            let htm = '<option value="-1" >ALL</option>';

            $.each(sortDeptName, function (key, item) {
...
function fillHOD_DDL() {
    $.ajax({
        url: "DashboardAdmin.aspx/GetHOD",
        ...
        success: function (result) {
            ...
            let htm = '<option>NO HOD AVAILABLE</option>';           
            hodList = result.d; 

            var sortHodList = [];
            hodList.sort(function (a, b) {
                return (a).EmpName.toUpperCase().localeCompare((b).EmpName.toUpperCase());
            })
            $.each(hodList, function (idx, itm) { sortHodList.push(itm) });


            if (hodList.length > 0) {
                htm = '';
                $.each(sortHodList, function (key, item) {
                    htm += key == 0 ? '<option value="' + item.EmpNo + '" selected>' + item.EmpName + '</option>' : '<option value="' + item.EmpNo + '">' + item.EmpName + '</option>';
                });
            } 
...
function FillManager_DDL() {
    $.ajax({
        url: "DashboardAdmin.aspx/GetEmployeesListByManager",
        data: JSON.stringify({ EmpNumber: selHOD }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        async: false,
        success: function (result) {
            ...
            var opt = '<option>NO MANAGER AVAILABLE</option>';
            var sortManList = sortManByName();

            if (lManagerList.length > 0) {
                opt = '<option>FILTER NOT INCLUDE</option>';
                $.each(sortManList, function (key, item) {
                    opt += key == 0 ? '<option value="' + item.EmpNo + '" selected>' + item.EName + '</option>' : '<option value="' + item.EmpNo + '">' + item.EName + '</option>';
            ...
            if (lManagerList.filter(s => s.IsManager != '').length > 0) {
                selLineMan = $('#DDLLineMan option:selected').val();
                //selUserId = lManagerList.filter(s => s.EmpNo == selLineMan)[0].UserId;      
            }
...
function fillEmployee_DDL() {

    $.ajax({
        url: "DashboardAdmin.aspx/GetEmployeesListByManager",
        data: JSON.stringify({ EmpNumber: selLineMan }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        async: false,
        success: function (result) {
            ...
            var opt =  '<option>NO EMPLOYEE AVAILABLE</option >';
