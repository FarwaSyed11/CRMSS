...
var dashboardObjectList = [], dashboardObjectList2 = [];
$(document).ready(function () {
    ...
    loadDDLCompany();
    loadDDLManager(getCompniesFromDDL()/*$('#selectTypeOpt option:selected').text().trim()*/, currUserId.val());
    loadDDLSal(currUserId.val(), getCompniesFromDDL()/*$('#selectTypeOpt option:selected').text().trim()*/, manId);
...
$('#btnSearchDashBoard').on('click', function () {
    $('#btnSearchDashBoard').prop('disabled',true);
    ...
    $('.wait-area-sign').removeClass('hidden');
...
$('#btnSototal').on('click', function () {
    ...
    let compId = getCompniesFromDDL();//$('#selectTypeOpt option:selected').text().trim();
...
function getAllDashboardDataValues() {
    ...
    loadForCountsAndValue2(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ...
    ////loadNumbersOfFinalized(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////loadNumberOfSalesForecast(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////loadPipline(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////loadFinalPrice(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////getCurrentForcastedOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////getFutureForcastedOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////getPendingForcastedOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);

    ////loadBChartValForFinalCard(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ////loadBChartValForPipelineCard(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
   ...
   //// getValueForProductOrValueBar(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ...
    //loadOpenOpportunities(compId, manId, salesmanId, currLoginUser, selectedYear, userStatus);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result.d, function (key, item) {
                if (key == 0) {
                    companyList += '<option value="bg-primary" selected>' + item.CompanyName + ' </option>';
                }
                else {
...
function loadForCountsAndValue2(compId, managerId, salesmanId, uId, year, uStatus) {
    $.ajax({
        url: "Dashboard.aspx/GetCountnvalForGCs2",
        ...
        async: true,
        success: function (result) {
            loadForCountsAndValue(compId, managerId, salesmanId, uId, year, uStatus);
            dashboardObjectList2 = result.d;
            ...
            //------------- Final Price Box value count ----------------------//
            $('.final-price-value-card').html(result.d.listFinalPriceBox[0].FinalPriceValue == "" ? 0 : nFormatter(result.d.listFinalPriceBox[0].FinalPriceValue));
            $('.final-price-value-card-count').html(result.d.listFinalPriceBox[0].FinalPriceCount == "" ? 0 : parseInt(result.d.listFinalPriceBox[0].FinalPriceCount).toLocaleString("en-US"));
                ...
                topOpp += `<li class="d-flex mb-4 pb-1">
                    ...
                    <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                        <div class="me-2" style="width:50%">
                            <small class="text-muted d-block mb-1">	`+ item.Owner + `</small>                         
                                <a href="`+ item.RedirectURL + `" class="mb-0">` + item.Name + `</a>
                        </div>
                        <div class="user-progress d-flex align-items-center gap-1">
            ...
            //-------------------- Top 50 Open JOH ------------------------------//            
            var topTen = '';
            $.each(result.d.listTop50Tender, function (key, item) {

                topTen += `<li class="d-flex mb-4 pb-1">
                    <div class="avatar flex-shrink-0 me-3">                   
                                                   <i class="bx bx-basketball listoprty"></i>
                    </div>
                    <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
            ...
            $('.top-opportunity-tender-ul').html(topTen);
...
function loadForCountsAndValue(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetCountnvalForGCs",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        async: true,
        success: function (result) {
            $('#btnSearchDashBoard').prop('disabled', false);
            ...
            $.each(result.d.listFirstRowRoundChrt, function (key, item) {
                ...
                if (item.For == "SO") {
                    ...
                    overAllPerfo += parseFloat(item.Overall == "" ? 0 : item.Overall);
                }
                else if (item.For == "INV") {
                ...
                else if (item.For == "JOH") {
                    arrAchiechedPercValueForGC4 = [];
                    arrAchiechedPercValueForGC4.push(parseInt(item.Perc));
                    arrForGCSaleEmp4.push(nFormatter(parseInt(item.Value)));
                    $('#g-chart-crms4').html(parseInt(item.Target));
                    overAllPerfo +=  parseFloat(item.Overall);
                }
                else if (item.For == "OTHERSYSTEM") {
                    arrAchiechedPercValueForGC5 = [];
                    arrAchiechedPercValueForGC5.push(parseInt(item.Perc));
                    arrForGCSaleEmp5.push(nFormatter(parseInt(item.Value)));
                    $('#g-chart-crms5').html(parseInt(item.Target));
                    overAllPerfo +=  parseFloat(item.Overall);
                }
                else if (item.For == "UPDATES") {
            ...
            $('.wait-area-sign').addClass('hidden');
...
$('#selectTypeOpt').on('change', function () {
    ...
    compId = getCompniesFromDDL();
    loadDDLManager(compId, currUserId.val());

});
$('#selectmanager').on('change', function () {

    compId = getCompniesFromDDL(); //$('#selectTypeOpt option:selected').text().trim();
...
function loadSalesCalForGC1(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetSalesCalForGC1",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC2(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetSalesCalForGC2",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC3(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetSalesCalForGC3",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC4(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetSalesCalForGC4",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC5(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetSalesCalForGC5",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadSalesCalForGC6(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetSalesCalForGC6",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function loadUserDetails(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetUserImage",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getValueForProductOrValueBar(compId, managerId, salesmanId, uId, year, uStatus) {
    ...
    $.ajax({
        url: "Dashboard.aspx/GetValForLostByCompitOrValue",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getCurrentForcastedOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    arrForecastOppChart = [];
    $.ajax({
        url: "Dashboard.aspx/GetCurrentForcastedOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getPendingForcastedOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    //arrForecastOppChart = [];
    $.ajax({
        url: "Dashboard.aspx/GetPendingForcastedOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getFutureForcastedOpportunities(compId, managerId, salesmanId, uId, year, uStatus) {
    arrFutureForecastOppChart = [];
    $.ajax({
        url: "Dashboard.aspx/GetFuturForcastedOpportunities",
        data: JSON.stringify({ "compId": compId, "manId": managerId, "salesmanId": salesmanId, "userId": uId, "year": year, "userStatus": uStatus }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
function getCompniesFromDDL() {
    var compnies = '';
    for (var i = 0; i < $('#selectTypeOpt option:selected').length; i++) {
        compnies += $('#selectTypeOpt option:selected')[i].text.trim()+',';
    }
